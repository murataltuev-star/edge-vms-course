...
**Creating a user touches no Node.** The UI talks to the signer's issuer; the issuer writes one row in the domain's identity store (or defers to the customer's IdP) and from then on signs tokens carrying that user's capabilities. Nothing is pushed to Nodes, because a Node never needs to know who exists — it needs to know whether *this* token is genuine and whether it names *this* camera, and both are a signature check against a public key it already holds plus a lookup in its own grants. The two things that do have to reach every cluster — the signer's current public key and the revocation list — are carried by **the domain agent**: one small Nomad job per cluster whose only right is to write `domain/*` in that cluster's Variables, the way a Node's only right is to write `nodes/<node>/*`. When the domain is down the agent stops updating, the Nodes keep verifying with the key they have, issued tokens run to expiry, and nobody new logs in — the bounded outage the services table already promises, with the mechanism named.
