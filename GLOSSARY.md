...
| Term | Meaning here |
|---|---|
| ~~**Node**~~ | *Retired (12 September 2026).* М9 Lessons 5–9's recorder: a process with its own database, cameras and archive index, distinct from the box it ran on. Under М10's shape what is on a server is the platform's stores, a *resource* on its disks and the *workers* placed there — and their sum is the *server*, so the fifth word went. Appears in М9 (history) and in М12's first-design text until that module is rewritten. |
...
| **Resource** (platform job) | One per server, pinned: the platform's `resource` job (`vmsplatform/resource.py`) heartbeats what is on the server's disks under `platform/resources/<server>`, serves every subsystem's buckets, mirrors closed ones to the next resource when `platform/mirror` is on, restores them after a replaced disk, retains by each subsystem's `<sub>/retention` rows, and runs the passes subsystems register on it (the VMS: manifests and media). |
...
| **Server** | A box with CPUs and disks — and, under М10's shape, everything on it: a replica of the cluster's stores, the platform's *resource* job on its disks, and whichever *workers* the scheduler places there. A Nomad *client*. М10 is ServerVMS: the platform's shape on one server. |
