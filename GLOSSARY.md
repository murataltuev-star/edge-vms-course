...
| **Event** | An observation — a detection, a silence, an operator's mark — written by the worker that made it, under its epoch, beside the segment it describes: `<cam>/e<epoch>/<start>Z.events.jsonl`. Promoted, counted, fenced and retained with the segment. No controller writes events; the cluster's `eventindex` is a cache over them (М10 L3, М11 L3). |
