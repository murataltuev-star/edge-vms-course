...
| Events | **Lines beside the segment** — `<cam>/e<epoch>/<start>Z.events.jsonl`, written by the worker that observed them while writing that segment; promoted first, counted in the manifest line, shown on the timeline, fenced by the same path, deleted by the same retention. **No controller writes events; an index over them (М11's `eventindex`) is a cache** *(12 September 2026)* | An event is an observation: append-only, keyed by camera and time, useless without the footage it points into — the manifest's shape, not a table's. A single writer of all events would be a serialization point on the hot path and a process on the recovery path of something continuous; the Node's own database is gone and does not come back for this |
