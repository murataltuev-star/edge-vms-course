...
NODEVMS_PATH=../../М10_NodeVMS/nodevms CLUSTERVMS_PATH=../clustervms ./measure.sh
```
...
| `ClusterStoreMixin` + М10's `PgStore` on asyncpg | `Store` interface; `PgStore` on pgx behind `-tags pg` | The measuring machine could not reach the Go module proxy, so `pgstore.go` was **type-checked against a stub of pgx's signatures, not run against Postgres**. The SQL is the Python version's, verified in М10 on Postgres 16. `go get github.com/jackc/pgx/v5 && go build -tags pg ./cmd/clustervms` is the missing step; without the tag the binary builds and refuses at start. |
| `urllib.request` to Nomad | `net/http` to Nomad | Still no `github.com/hashicorp/nomad/api` (MPL-2.0 — the note from М10 Lesson 5 stands): the four calls the Node makes are forty lines. |

Two things are not here, and were not in the Python `clustervms/` either: the GStreamer actuator (М10's, and in a Go controller it becomes a *client* of a C++ media worker — Lesson 5's per-frame rule survives cgo) and М10's retention task and console. `cmd/clustervms` runs the Node with the fake actuator: everything about identity, restore, epoch, lease, directory and metrics is real; nothing records.

## The numbers (measured, Linux x86-64, Go 1.24.7, Python 3.11)

Both processes are the **whole Node at idle**: fifty cameras restored from the directory, epoch taken, every task running (reconcile, report, publish, lease, heartbeat, re-index; in Python also М10's bus pump and retention), a console listener, fakes for Nomad and Postgres, a directory for the object store. PSS from `/proc/self/smaps_rollup`, three runs each.

| | Go | Python | |
|---|---|---|---|
| **Node at idle, 50 cameras, every task running** | **7.1 MB** (10 goroutines) | **28.5 MB** (11 asyncio tasks) | 4.0× — the same ratio М10 measured on the reconciler alone (6.0 vs 25.7). The cluster layer added 1 MB in Go and 3 MB in Python. |
| Deployable artifact | one static binary, **6.6 MB** (arm64: 6.2 MB, one `GOARCH=arm64` away) — without pgx | interpreter + asyncpg + the rest, as М10 counted them | |
...
**The win is memory and deployment, not speed** — which is what М10 Lesson 5 predicted. A cluster controller's work is reading a Variable, hashing an object, and comparing two maps; Python does that within 2× of Go, and hashing within 20 %, because the hot part of each is already C. What Python cannot shed is the 20 MB it costs to be Python, and the interpreter-plus-wheels rootfs that М9's bundle has to carry. On a Node whose budget is `B + n·I`, four times smaller `B` is more cameras per box; on an appliance whose update is a RAUC bundle, the controller's whole contribution to the bundle becomes one 6 MB file with no interpreter to ship, cross-built for arm64 in one command.
...
**What this does not settle** is the media worker, and it does not try to: the per-frame rule from М10 Lesson 3 holds in Go exactly as it holds in Python, and the worker is C++ either way.
