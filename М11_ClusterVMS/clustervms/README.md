...
clustervms/
  cluster/
    ...
    identity.py      L26 — who am I: the Variable the scheduler delivered, never the allocation index
    epoch.py         L28 — next_epoch by CAS; Lease on a monotonic clock, renewal = reading my own epoch
    configio.py      L27 — what travels: the configuration as one blob; dump/restore on М10's PgStore
    publish.py       L27 — object first, then the Variable; publish on change with a floor; `replicated`
    rehydrate.py     L27 — the six steps; `unconfigured`; a dangling pointer refused
    directory.py     L29 — scan nodes/*: where is camera 7
    placement.py     L29 — capacity measured, constraints as labels, placement STORED in placement/<camera>, budgeted rebalance
    metrics.py       L28 — node_failover_seconds{kind="worst"}, node_epoch_conflicts, appended to М10's /metrics
  ...
  deploy/
    server.hcl, client.hcl       L25 — three servers, ACLs on, data_dir on /data, meta.vlans, the Podman plugin
    render.py                    L26 — one jobspec per Node (job name = identity), its ACL policy, its bootstrap Variables
    node-3-policy.hcl            L26 — one writer per key
    minio.nomad.hcl              L27 — the object store on the cluster's own servers
    Containerfile                the image the job runs: nodevms + cluster
  tools/place.py                 L29 — the placement service as a command
  tests/                         24 tests, no Nomad, no Postgres, milliseconds: python3 tests/run.py
```
...
- **Run, output real:** `tests/run.py` — 24 tests against the in-memory
