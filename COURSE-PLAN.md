...
| 3 | **Postgres** | What does this system know about itself? | М10 | Planned |
| 4 | **Domain controller** | Cameras, archives, detectors — the actual product | М11 | Planned |
...
### М10 — State: Postgres · ~4 lessons

The cloud VMS spec forbade a database outright. The appliance needs one, and understanding *why the answer flipped* is the point of the module: in the cloud, KVS held the configuration; on-prem, the box holds it. Cameras, retention policies, schedules, users — that is domain state, and it has to live somewhere that survives.

- Schema for cameras, archives, retention; migrations as a shipped artifact
- Postgres as a Nomad job vs. on the host, and why the answer is not obvious on an appliance
- **The critical one:** the database lives on the data partition, so it survives A/B OS updates untouched. This is М9's three-way boundary with real consequences
- Backup and restore on a box nobody visits

### М11 — The domain controller · ~6 lessons

Where the course stops being about infrastructure and starts being about the product.

- A reconciliation loop: desired camera state in Postgres, actual pipelines running under Nomad, and the controller closing the gap. Students have already met this idea as GitOps — now they write it
- Camera lifecycle: added, unreachable, removed, credentials rotated
- Archive and retention enforcement
- Detector attachment, and where inference actually runs
- The API the console talks to

The largest module, and the most specific to the product rather than the stack.
...
- **М10 before М11** — the controller needs somewhere to keep state
...
|---|---|---|
| М8 — Cloud VMS | 15 | 15 |
| М9 — Edge appliance | 9 | 24 |
| М10 — Postgres | ~4 | ~28 |
| М11 — Domain controller | ~6 | ~34 |
| М12 — OpenBao | ~4 | ~38 |
...
1. **The BUSL decision, taken once.** If Nomad is unacceptable in a shipped product, that changes М9 and everything above it. Decide before М10, not after М14
...
3. **Where inference runs** — on the appliance, at the camera, or in the cloud. Shapes М11 substantially
4. **М13's position** — before or after the domain controller
5. **Lesson numbering.** М9 assumes 16–24; М10 onward continues from 25
