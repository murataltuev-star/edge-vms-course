...
> **Scope note.** These lessons were briefly М9's Part B and then briefly М10's Part B before landing here. The last move happened because a cluster and the layer above it are one arc, and splitting them meant teaching the two-level idea twice. Nomad's cross-site federation went further still, to [М12](../М12_OrchestratedVMS/module-design.md), where many networks actually begin. The write API is built **deliberately unauthenticated** in Lesson 32 and given a channel and a credential in Lesson 33 — both hand-provisioned, both marked temporary, exactly as М10's database password was.
